...
JSONB_KEYS=$(grep -v '^$' "$APP_DIR/helper_data/jsonb_field_keys.txt" | sed "s/.*/'&'/" | paste -sd, -)
JSONB_PREFIXES=$(grep -v '^$' "$APP_DIR/helper_data/jsonb_field_prefixes.txt" | awk '{print "OR key LIKE \x27" $0 "%\x27"}' | paste -sd' ' -)

FIELD_DEFS="$(grep -v '^$' "$APP_DIR/helper_data/jsonb_field_keys.txt" | sed 's/.*/"&":"String"/' | paste -sd, -)"
FIELD_DEFS="$FIELD_DEFS,$(grep -v '^$' "$APP_DIR/helper_data/jsonb_field_prefixes.txt" | sed 's/.*/"&\*":"String"/' | paste -sd, -)"
